# pages/map_view.py (CORRIGIDO - Adicionado link nos blocos de resumo)
...
def get_layout():
    ...
    try:
        layout = dbc.Container([
            dbc.Row([dbc.Col(html.H2("Mapa de Monitoramento - Rod. Tamoios"), width=12, lg=8), dbc.Col(
                dbc.Button("Ver Dashboard Geral", color="primary", href="/dashboard-geral", size="lg",
                           className="w-100"), width=12, lg=4, className="my-2 my-lg-0")],
            ...
            dbc.Row([dbc.Col(
                html.Div([
                    dl.Map(
                        ...
                        children=[
                            dl.TileLayer(),
                            dl.LayerGroup(id='map-pins-layer'), # Camada para pinos
...
# (Mantido idêntico)
CHUVA_LIM_VERDE = 50.0; CHUVA_LIM_AMARELO = 69.0; CHUVA_LIM_LARANJA = 89.0
def get_color_class_chuva(value):
    if pd.isna(value): return "bg-secondary";
    if value <= CHUVA_LIM_VERDE: return "bg-success";
    elif value <= CHUVA_LIM_AMARELO: return "bg-warning";
    elif value <= CHUVA_LIM_LARANJA: return "bg-orange";
    else: return "bg-danger"
RISCO_MAP = {"LIVRE": 0, "ATENÇÃO": 1, "ALERTA": 2, "PARALIZAÇÃO": 3, "SEM DADOS": -1}

# --- Função auxiliar create_km_block (MODIFICADA para incluir Link) ---
def create_km_block(id_ponto, config, df_ponto):
    # ... (cálculos iniciais mantidos idênticos) ...
    ultima_chuva_72h = 0.0;
    umidade_1m_atual = 0.0; umidade_2m_atual = 0.0; umidade_3m_atual = 0.0
    base_1m = config.get('constantes', {}).get('UMIDADE_BASE_1M', 0.0);
    base_2m = config.get('constantes', {}).get('UMIDADE_BASE_2M', 0.0);
    base_3m = config.get('constantes', {}).get('UMIDADE_BASE_3M', 0.0)
    ...
    try:
        if not df_ponto.empty:
            ...
            try:
                ...
                umidade_1m_atual = ultimo_dado.get('umidade_1m_perc', 0.0);
                umidade_2m_atual = ultimo_dado.get('umidade_2m_perc', 0.0);
                umidade_3m_atual = ultimo_dado.get('umidade_3m_perc', 0.0)
                if pd.isna(umidade_1m_atual): umidade_1m_atual = 0.0
                if pd.isna(umidade_2m_atual): umidade_2m_atual = 0.0
    ...
    # --- INÍCIO DA ALTERAÇÃO: Envolver o conteúdo com html.A ---
    ...
    ], className="km-summary-block") # Mantém a classe original para estilo

    # Retorna o conteúdo envolvido pelo link
    return html.A(
        conteudo_bloco,
        href=link_destino,
        style={'textDecoration': 'none', 'color': 'inherit'} # Remove estilo padrão de link
    )
    # --- FIM DA ALTERAÇÃO ---
    ...
    try:
        ...
        for id_ponto in ids_esquerda:
            if id_ponto in PONTOS_DE_ANALISE:
                ...
                df_ponto = df_completo[df_completo['id_ponto'] == id_ponto]
                km_block = create_km_block(id_ponto, config, df_ponto) # Agora retorna um html.A
